...
def printDot(op=None,extra=None): pass # dummy code

def _makeRecon_SVD_only(spM, dm, dmOrder, lambd, showSteps=False):
   return {'rmx':
      numpy.linalg.pinv( spM.T.dot(spM), lambd[0]*spM.max()**-1.0 ).dot(spM.T)}

def _makeRecon_InvPlusTik(spM, dm, dmOrder, lambd, showSteps=False):
   dmLength=sum([ sum(dm[k].usable) for k in dm.keys() ])
   regM=numpy.identity(dmLength)*lambd[0]
   return {'rmx': numpy.linalg.inv( spM.T.dot(spM)+regM ).dot(spM.T) }

def _makeRecon_DR(spM, dm, dmOrder, lambd, showSteps=False):
   """Diagonal-regularized
   ...
   sTsM=spM.T.dot(spM)
   dmLength=sum([ sum(dm[k].usable) for k in dm.keys() ])
   regM=numpy.identity(dmLength)*lambd[1]
   fTfM=filterM.T.dot(filterM)
   return {'rmx': numpy.linalg.inv( sTsM+fTfM+regM ).dot( spM.T ) }

def _makeRecon_LOP(spM, dm, dmOrder, lambd, showSteps=False):
   """Low-order-penalization
            ...
            filterM[
               ...
               (dmOrder[0]=='lo')*sum(dm['lo'].usable)
                +sum(dm['ho'].usable)
                  ...
                  lambd[0]**0.5*trigFn( 2*lambd[1]**-1.0*numpy.pi*hodmN**-1.0*(
                     xf*hoDMactCds[0]+yf*hoDMactCds[1]) )).take(
                        dm['ho'].usableIdx )
            modeCounter+=1
   fTfM=filterM.T.dot(filterM)
   sTsM=spM.T.dot(spM)
   return {'rmx': numpy.linalg.pinv( sTsM+fTfM).dot(spM.T) }

def _makeRecon_PMXf(spM, dm, dmOrder, lambd, showSteps=False):
   """PMX filtering, explicit via Cholesky decomposition
   ...
   printDot(showSteps)
   loPMX=spM.take(range(sI['lo'],eI['lo']),axis=1)
   hoPMX=spM.take(range(sI['ho'],eI['ho']),axis=1)
...
def _makeRecon_PMXf2(spM, dm, dmOrder, lambd, showSteps=False):
   """PMX filtering, implicit via SVD
      Construct the following stacked reconstruction matrix:
      PMX=(PMX_HO,PMX_LO) -> RMX=(RMX_HO(I-PMX_LO.PMX^-1_LO),RMX_LO)
   """
   sI={};eI={}
   for dmType in 'lo','ho':
      sI[dmType],eI[dmType]=[
   ...
   hoPMX=spM.take(range(sI['ho'],eI['ho']),axis=1)
   printDot(showSteps)
   loRMX=numpy.linalg.pinv(loPMX,lambd[0]).astype(numpy.float32)
   printDot(showSteps)
   hoRMX_F=numpy.linalg.pinv(hoPMX,lambd[1]).dot(
      numpy.identity(hoPMX.shape[0])-loPMX.dot(loRMX) 
         ).astype(numpy.float32)
   rmx=numpy.empty( spM.shape[::-1], numpy.float32 )
   rmx[ range(sI['lo'],eI['lo']) ]=loRMX
   rmx[ range(sI['ho'],eI['ho']) ]=hoRMX_F
   return { 'hoRMX':hoRMX_F, 'rmx':rmx }

def _makeRecon_HOO(spM,dm,dmOrder,lambd, showSteps=False):
   """HO_only
      Construct the following stacked reconstruction matrix:
      PMX=(PMX_HO,0) -> RMX=(PMX_HO^-1,0)
   """
   sI,eI=[ sum([ numpy.sum(dm[dmOrder[i]].usable)
         for i in range(dmOrder.index('ho')+j) ]) for j in (0,1) ]
   hoPMX=spM.take(range(sI,eI),axis=1)
   hoRMX=numpy.linalg.pinv(hoPMX,lambd[0])
      # \/ make a blank array, and then fill in only part of it
   reconM=numpy.zeros( spM.shape[::-1], numpy.float32 )
   reconM[sI:eI]=hoRMX
   return { 'rmx': reconM }

def _makeRecon_HOOB(spM,dm,dmOrder,lambd, showSteps=False):
   """HO_onlyB
      Construct the following stacked reconstruction matrix:
      PMX=(PMX_HO,0) -> RMX=(PMX)^-1
   """
   sI,eI=[ sum([ numpy.sum(dm[dmOrder[i]].usable)
         for i in range(dmOrder.index('lo')+j) ]) for j in (0,1) ]
      # \/ copy the PMX and zero out the low-order pokes
   stackedHOOnlyPMX=spM.copy()
   stackedHOOnlyPMX[:,sI:eI]=0
   return { 'rmx': numpy.linalg.pinv(stackedHOOnlyPMX,lambd[0]) }



knownAlgorithms={
   'SVD_only':_makeRecon_SVD_only,
   'Inv+Tik':_makeRecon_InvPlusTik,
   'Diagonal-regularized':_makeRecon_DR,
   'Low-order-penalization':_makeRecon_LOP,
   'PMX_filtering':_makeRecon_PMXf,
   'AGB-PMX_filtering':_makeRecon_PMXf2,
   'HO_only':_makeRecon_HOO,
   'HO_onlyB':_makeRecon_HOOB,
   }


def makeReconstructor(reconTypes, stackedPMX, dm, dmOrder,
      showSteps=False):
   reconMs={}
   for reconType,lambd in reconTypes:
      printDot(showSteps,
            "makeReconstructor: Making '{0:s}' reconstructor".format(
               reconType))
      if reconType in knownAlgorithms:
         reconMs[reconType]=knownAlgorithms[reconType](
               stackedPMX,dm,dmOrder, lambd, showSteps)
      else:
         print("Known algorithms are:")
         for k in knownAlgorithms: print("\t"+k)
         raise ValueError("Unknown algorithm") 
      printDot(showSteps,"done")
   return reconMs

makeReconstructors=makeReconstructor # for compatibility
...
if __name__=="__main__":
   ...
   def generateTestPhase(gO,r0,L0,N):
   ...
   L0=nSubAps*4           # relative outer-scale
   reconTypes=(
      "SVD_only",
      "Diagonal-regularized",
      "Low-order-penalization",
   ...
   try:
      ...
      reconTypeIdx=1        # which reconstructor to use (makes several types)
      doPlotting=True
   ...
   directTestPhase=generateTestPhase(gO,nSubAps/32.0,L0,nSubAps+1) # test phase
   ...
   if reconTypeIdx==-1:
      raise RuntimeError("You asked me to stop, so I shall.")

   # >>> The following three lines show how to use the function
   # >>>
   reconT=( reconTypes[reconTypeIdx],lambds[reconTypeIdx] ) # configure
   reconMs=makeReconstructor([reconT], stackedPMX, dm, dmOrder, showSteps=1)
