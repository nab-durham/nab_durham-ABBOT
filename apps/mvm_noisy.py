...
noiseReductionM=numpy.load("mvm_noisy_nrM.pickle")
NoisygradV=numpy.dot(noiseReductionM,
   gradV+numpy.random.normal(0,gradV.var()**0.5,size=gO.numberSubaps*2)) # add noise, then remove it
...
for thisRM in reconM:
   if thisRM!=None:
      reconPhaseV.append( numpy.dot( thisRM, NoisygradV ) )
   else:      
